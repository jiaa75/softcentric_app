...
MISC_FILE = "data/misc_expenses.json"
...
for file in [USERS_FILE, PROJECTS_FILE, EXPENSES_FILE, PROGRESS_FILE]:
...
def dashboard():
    ...
    messages = read_json(MESSAGES_FILE)  # Ensure this file exists

    # Check if user requested to see completed projects
    show_completed = request.args.get("completed", "").lower() == "true"

    # ----------------- User-restricted projects -----------------
    if role != "admin":
        projects = [p for p in all_projects if username in p["users"]]
        # Filter expenses and progress for assigned projects
    ...
    # ----------------- Filter in-progress or completed projects -----------------
    if show_completed:
        filtered_projects = [p for p in projects if p.get("status", "").lower() == "completed"]
    else:
        filtered_projects = [p for p in projects if p.get("status", "").lower() != "completed"]

    # ----------------- Unread messages count -----------------
    unread_count = sum(1 for m in messages if m["receiver"] == username and not m.get("read", False))
...
# ------------------ Updates to project_add to set default status ------------------
...
# ------------------ Updates to project_edit to preserve status ------------------
...
def project_edit(project_id):
    ...
    if request.method == "POST":
        ...
        # Preserve status field; no changes here
        write_json(PROJECTS_FILE, projects)
        return redirect(url_for("dashboard"))
    users = read_json(USERS_FILE)
...
def project_detail(project_id):
    ...
    # Get expenses and progress for this project
    ...
    # Ensure every progress dict has instructions key
    for p in progress:
...
def view_expense():
    ...
    # Role-based filtering: non-admin sees only their projects
    if role != "admin":
...
def add_progress():
    ...
    if request.method == "POST":
        ...
        new_progress = {
            ...
            "date": request.form["date"],
            "user": username  # store current user
        }
...
def view_progress():
    ...
    # Decide which projects dict to send to template
    if show_completed:
...
def messages():
    ...
    # Build conversations grouped by correspondent
    conversations = {}
    unread_counts = {}  # NEW: store unread messages per user
    for msg in messages:
        if current_user in [msg["sender"], msg["receiver"]]:
            ...
            # Increment unread only if the message is received by current user and unread
    ...
    return render_template(
        ...
        all_users=all_users,
        unread_counts=unread_counts  # Pass unread counts to template
    )
...
def chat_with(receiver):
    ...
    conversation = [m for m in messages if (m["sender"] == username and m["receiver"] == receiver) or
                                        (m["sender"] == receiver and m["receiver"] == username)]
...
def unread_details():
    username = session["username"]
    messages = read_json(MESSAGES_FILE)
    # Only unread messages received by the user
    unread = [m for m in messages if m["receiver"] == username and not m.get("read", False)]
...
def view_misc_expense():
    ...
    # Filter by role: non-admin sees only their own expenses
    if role != "admin":
    ...
    # Filter current vs previous months
    if show_previous:
    ...
    if month_filter:
        misc_expenses = [e for e in misc_expenses if e["date"][:7] == month_filter]  # YYYY-MM
